...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        self.channel_per_joint = extra.CHANNEL_PER_JOINT  # 28(resnet 50)
        ...
        # 根据Rtmpose，这里的kernal_size=7时效果最好
        # https://github.com/open-mmlab/mmpose/blob/dev-1.x/configs/body_2d_keypoint/rtmpose/coco/rtmpose-m_8xb256-420e_coco-256x192.py
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)  # torch.Size([32, 2048, 8, 6])

        x = self.final_layer(x)  # torch.Size([32, 476, 8, 6])

        x = rearrange(x, 'b (k t) h w -> b k (t h w)',k=self.num_joints,t=self.channel_per_joint)  # torch.Size([32, 17, 1344])

        pred_x = self.mlp_head_x(x)  # torch.Size([32, 17, 384])
        pred_y = self.mlp_head_y(x)  # torch.Size([32, 17, 512])
