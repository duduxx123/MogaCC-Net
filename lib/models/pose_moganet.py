...
class PoseMoganet(nn.Module):
    def __init__(self,cfg , **kwargs):
        super().__init__()
        extra = cfg.MODEL.EXTRA
        self.num_joints = cfg.MODEL.NUM_JOINTS
        self.simcc_split_ratio = cfg.MODEL.SIMDR_SPLIT_RATIO
        self.moganet = moganet_tiny(pretrained=False,fork_feat=True,drop_path_rate=0.15,stem_norm_type='SyncBN',conv_norm_type='SyncBN')
        self.in_channel = 256  # moganet最后输出的特征图通道数
        ...
        self.mlp = nn.Sequential(
            ScaleNorm(flatten_dims),  # 使用GAU模块用这个归一化  mAP 73.2
            # nn.LayerNorm(flatten_dims),  # 不用GAU时用这个，mAP 73.0
        ...
        self.mlp_head_x = nn.Linear(gau_cfg['hidden_dims'], W, bias=False)
        self.mlp_head_y = nn.Linear(gau_cfg['hidden_dims'], H, bias=False)
        ...
        #     kernel_size=extra.FINAL_CONV_KERNEL,  # 这里设置为1
        #     stride=1,
...
if __name__ == '__main__':
    '''
    在进行人体姿态估计时，moganet后面会接head，head定义在mmpose中
    在moganet_t_coco_256x192.py中对head的类型进行了定义，为TopdownHeatmapSimpleHead
    以下是TopdownHeatmapSimpleHead的代码实现：
    https://github.com/open-mmlab/mmpose/blob/0d9a00c1d64b68999e4b2a4ee5ed0d5ac2c3e296/mmpose/models/heads/topdown_heatmap_simple_head.py
    '''
    from thop import profile
    x = torch.rand((1, 3, 256, 192))
    # model = MogaNet(arch='t',fork_feat=True)
    model = PoseMoganet()  #
    # model.default_cfg = default_cfgs['moganet_t']
